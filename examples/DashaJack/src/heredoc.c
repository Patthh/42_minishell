...
void	set_heredoc_fds(int pipe_fds[2], t_var **envp_list)
{
	int		stdout;
			...
			t_var **envp_list, t_exec_state *state)
{
	...
	if (!isatty(STDOUT_FILENO))
		set_heredoc_fds(pipe_fds, envp_list);
	while (1)
	{
		line = readline("> ");
		if (ft_strcmp(line, tree->left->data) == 0)
		{
			free(line);
			close(pipe_fds[1]);
			free_all(tree, *envp_list, state, 0);
		}
...
int	redirect_heredoc(t_tree *tree, t_var **envp_list, t_exec_state *state)
{
	...
	{
		close(pipe_fds[1]);
		redirect_input(NULL, &pipe_fds[0], envp_list);
