...
int	execute_redirections(t_tree *tree, t_var **envp_list, t_exec_state *state)
{
	if (tree->left && tree->type == LESS)
		state->status = redirect_input(tree, NULL, envp_list);
	else if (tree->left && tree->type == DLESS)
		redirect_heredoc(tree, envp_list, state);
	else if (tree->left && tree->type == GREATER)
		state->status = redirect_output(tree, envp_list, state);
	else if (tree->left && tree->type == DGREATER)
		state->status = redirect_output_append(tree, envp_list, state);
	else
		return (0);
	return (1);
}
	...
	int pid[MAX_PIDS])
{
	...
	while (tree)
	{
		...
		if (tree->type == PIPE)
			pid[i++] = execute_pipe(tree, envp_list, state, &is_p);
		else if (tree && tree->type == CMD)
		{
			if (state->outfile != -1)
				redirect_output_to_pipe(NULL, envp_list, state);
			if (is_p == 1)
				pid[i++] = execute_last(tree, tree->data, envp_list, state);
			else if (!is_builtin(tree->data, tree, envp_list, state))
				pid[i++] = execute_last(tree, tree->data, envp_list, state);
		}
		if (check_pipeline_status(&tree, state, &is_p, envp_list) == -1)
			return (i);
	}
	close(STDIN_FILENO);
	return (i);
