...
/*   Updated: 2024/10/11 19:46:43 by dsiroten         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		free_node(t_link_list *node);
void		free_list(t_link_list **current);
char		*ft_join(t_link_list **current);
int			redirect_output_to_pipe(int *fd, t_var **envp_list,
				t_exec_state *state);
int			expand_var(char **input, char *arg, int *arg_index,
				t_var *envp_list);
char		*get_filename(char *input, t_var **envp_list);
char		*get_home(t_var *envp_list);
char		*get_next_line(int fd);
int			redirect_input(t_tree *tree, int *fd, t_var **envp);
int			redirect_output(t_tree *tree, t_var **envp, t_exec_state *state);
int			redirect_output_append(t_tree *tree, t_var **envp,
				t_exec_state *state);
...
int			check_pipeline_status(t_tree **tree, t_exec_state *status,
				int *is_pipe, t_var **envp);
int			empty_pipe(t_var **enpv);
