...
int	empty_pipe(t_var **envp_list)
{
	int	fds[2];
	...
	close(fds[1]);
	return (redirect_input(NULL, &fds[0], envp_list));
}

int	check_pipeline_status(t_tree **tree, t_exec_state *state,
	int *is_pipe, t_var **envp_list)
{
	if (state->status == 0)
	{
		*tree = (*tree)->right;
